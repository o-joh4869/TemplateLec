...
これを解決するためにテンプレートパラメータを増やして解決する。  
...
実用上二つのパラメータの最大を取りたい時、テンプレートパラメータを明示的に指定するのは面倒だ。  
...
大抵の場合通常の関数と同様だが、コンストラクタなどの特殊関数に関しては事情が違う。  

## コンストラクタテンプレート
コンストラクタをテンプレート化した場合、テンプレート引数の明示的指定ができないため引数から推論できるもののみ使用可能である。  
通常、暗黙に型変換が定義された型同士のコピーを許すときなどによく使われる。  
```cpp
template<typename T>
class Hoge {
public:
...
## Tipps : ファイル分けと特殊化
テンプレートのファイル分けはやや面倒なことが絡むため後に詳しく紹介する。  
今は特殊化も含めて1つのヘッダーファイルにすべて書くことを勧める。  
...
# 可変引数テンプレート
引数の数や型がわからず、引数を可変長にしたい場面がしばしばある。  
...
複数(0個を含む)のパラメータを含むArgsやargsをパラメータパックという。  
このパラメータパックを展開するには
```
args...
```
...
パラメータパックの値をそのまま用いることは基本的になく、関数に渡してしまうことがほとんどである。  

一つ一つの値を用いて何かをしたい場合、別の関数テンプレート等を用いて再帰的に解決する。  
```cpp
//可変引数の最大値を返す関数
template<typename T, typename... Args>
const T &max(const T &t, const Args&... args) {
...
# 発展問題  
1. 演習問題2で作ったクラスを以下の要件で拡張し挙動を確認せよ。  
...
・動的に要素数が指定できるようにコンストラクタを変更する。  
2. 要素を管理するクラスにstd::listが指定されたとき、operator[]がstd::listのメンバに存在しないためエラーになってしまう。  
明示的特殊化を用いてこれを解決せよ。  
3. この方法では静的に要素数が決定する場合の特殊化を十分に実現できない。  
動的静的の双方で要素数を指定できる方法を実装せよ。  
4. 静的な場合の要素数が2個だった場合について特殊化を行い、見かけの先頭要素の配列番号をbool型で管理せよ。  