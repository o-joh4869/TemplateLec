...
```
と書かれている。  
これによってprintArray関数の仮引数tの型を仮の型として残したまま定義が書ける。  
このTをテンプレートパラメータと呼ぶ。  
...
インスタンス化された関数(クラス)と明示的特殊化において作成された関数クラスを合わせて特殊化という。  
...
```
とすることができる。  
しかし、このクラスの中でCONTを利用するとき毎回CONTの第二引数に```std::allocator<T>```と書かなければいけないのは実用上面倒である。  
